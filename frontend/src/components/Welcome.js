...
const Welcome = ({ setGameState, savedUsername, onUsernameSet }) => {
  ...
  // Web3 hooks
  const { address, isConnected, open, disconnect } = useAccount();
          ...
          <button
            onClick={() => {
              ...
              open().catch(err => {
                ...
                const userCancelled = message.includes('rejected') || message.includes('denied') || message.includes('canceled') || message.includes('cancelled');
                if (!userCancelled) {
